...
<%		
		...
		if(manid==1){
			...
			for(ManagerVO m: user){
				List<ManagerVO> sublist =mSvc.getAll().stream()
						.filter(p -> p.getMan_id()==m.getMan_id())
						.collect(Collectors.toList());
				for(ManagerVO mvo : sublist){
					mlist.add(mvo);
				}
			}
...
// 			pageContext.setAttribute("wlist", wList);
			...
			for(ManagerVO m: user){
				int man_id=(int) m.getMan_id();
				System.out.println("man_id@ wList:" + man_id);
				List<WorkdateBean> subList = wSvc.selectAll().stream()
						.filter(p -> p.getMan_id()==man_id)
						.collect(Collectors.toList());
				for(WorkdateBean w : subList){
					wList.add(w);
				}
			}
